...
### 빠른 효과 (낮은 난이도)
1. ~~**섹터 분산 캡**~~ — ⚠️ **구현·측정 완료, 효과 없음.** `max_per_sector`로 섹터당 보유
   상한을 걸어봤으나 백테스트에서 낙폭이 오히려 악화(-25.4%→-27.6%/-29.0%). 이 전략의
   낙폭은 **모멘텀 팩터 드로다운**(고베타 종목 섹터 무관 동반 하락)이라 섹터 분산이 안 통함.
   기본 OFF로 되돌림. 기능은 추천 다양성용으로 유지. → 낙폭 추가 축소는 **저베타 유니버스/
   팩터 중립화/전략 변경**이 필요(단순 오버레이로는 한계).
2. **ATR 기반 포지션 한도/스톱** — 보유당 ATR 배수로 손절·사이징 → 꼬리 손실 축소.
3. **종목별 최대 비중 캡** — 앙상블이 핫한 한 종목에 과집중하지 않도록 10~15% 하드캡.

### 중간 난이도 (알파 기여 큼)
4. ~~**벤치마크 상대강도(RS)**~~ — ✅ **구현됨** (`relative_strength` 전략). 종목 수익률 −
   동일시장 유니버스 평균 → 시장을 이기는 종목에 가점. 기본 활성.
...
1. ~~**과거 추천 성과 추적**~~ *(최고 가치)* — ✅ **구현됨** (`performance/` + `track` 명령).
   `run-once`가 추천을 원장에 기록 → 이후 가격으로 방향조정 전방수익·적중률을 기간별/액션별로
   채점하고 확신도 유효성(≥0.7 vs <0.7)까지 비교. 기존 리포트 자동 백필.
2. ~~**포지션별 스톱 경보**~~ — ✅ **구현됨** (`evaluate_position_stop` + 리스크 스캔 + `risk` 명령).
   보유가 평단 대비 −15%(하드 스톱) 또는 최근고점 −2.5×ATR(추세 이탈) 이탈 시 슬랙 경보.
3. ~~**포트폴리오 조언 백테스트**~~ — ✅ **구현됨** (`track --portfolio`). 보유 조언(ADD/TRIM/SELL)을
   `portfolio_*.json`에서 백필해 이후 가격에 리플레이·채점.
